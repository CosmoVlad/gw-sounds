...
for i in range(N):
    chunks.append(signal[i*n:(i+1)*n])
    times.append(time[i*n:(i+1)*n])
...
for chunk in chunks:
    chunk += noise_ampl * np.max(chunk) * rng.normal(size=len(chunk))
    data.append(chunk)
    noise_ampl *=2
        
data = np.concatenate(data)

data = amplitude * data/np.abs(data).max()
