...
for i in [0.,*np.arange(5)]:
    
    numsignals = int(2**i)
    
    print(numsignals)
    
    mm = 10**(2 + 2*rng.random(numsignals))
    ff = 10**(-4 + 2*rng.random(numsignals))

    time,chunk = gen_sounds(duration, ff, mm, sr=samplerate)
...
full_chunks = np.cumsum(chunks, axis=0)

# for chunk in np.cumsum(chunks, axis=0):
...
for (x,y),line in zip(ffts,p):
    
    fig,ax = plt.subplots(ncols=1, nrows=1, figsize=(7,5))
    
    ax.semilogx(
        x, y, 
        lw=2, c=line[0].get_color()
    )
